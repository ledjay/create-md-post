  ...
  const configPrompts = await p.group(
    {
      ...
      path: () =>
        p.text({
          ...
          initialValue: "/posts",
          validate(value) {
            if (value.length === 0) return `I need a folder!`;
          },
        }),
      ...
      author: () =>
        p.text({
          message: `${getLingo(
            "okays"
          )} What's your Author's name for this blog?`,
      ...
      outro: ({ results }) => {
        p.outro(
          `Okay dear ${String(
            styles.highlight(results.author)
          )}, you will find your ${String(
            styles.highlight(results.extention)
          )} files inside the ${String(styles.highlight(results.path))} folder.`
        );
      },
    },
    {
